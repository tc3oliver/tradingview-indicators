...
| [btc-microstructure](./btc-microstructure/) | **BTC Order Book / Execution Intelligence**, plus the study that led to it. **Study M1** (rejected): 53.2 million raw Binance aggTrades testing whether aggressive trade flow predicts short-horizon returns — both hypotheses rejected; what survives is a mild *contrarian* signature worth 0.36 bp against a 14 bp round trip. **Execution Planner** (working, `npm start`): a local read-only tool that walks the live validated order book and tells you what a market order costs right now — spread, VWAP, slippage, fee, all-in cost — with no directional claim. **Study M2** (pre-registered, collecting): whether the order book predicts the next 30 seconds, with a production L2 collector, a 30-day minimum sample gate, and a research runner that refuses to compute a verdict below it |
